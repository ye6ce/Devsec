  ...
  q += toward * uMouseInfluence * 0.2;
    ...
    if (uColorCount > 0) {
      ...
      for (int i = 0; i < MAX_COLORS; ++i) {
            ...
            vec2 r = sin(1.5 * (s.yx * freq) + 2.0 * cos(s * freq));
            vec2 warped = mix(s, s + (r - s) * gain, warp);
            
            float m = length(warped + sin(5.0 * warped.y * freq - 3.0 * t + float(i)) * 0.25);
            
            // Faster weight calculation
            float w = clamp(1.0 - m * 1.5, 0.0, 1.0);
            w = w * w * (3.0 - 2.0 * w); // Smoothstep-like
