...
void main(){
  ...
  float luma = (rC + gC + bC) / 3.0;
  luma = (luma - 0.5) * u_contrast + 0.5;
  
  // Base is white, liquid intensity (luma) brings in the pink
  vec3 pink = u_tint;
  vec3 white = vec3(1.0);
  
  // The logo is white, and the "liquid" (the moving highlights/shadows) is pink
  // We use luma to determine the liquid intensity
  float liquidIntensity = smoothstep(0.2, 0.8, luma);
  
  // Mix pink and white. Higher luma (peaks) will be white, lower luma (valleys) will be pink
  vec3 col = mix(pink, white, liquidIntensity);
  
  // Add a subtle white highlight on the very top of the peaks
  col = mix(col, vec3(1.1), smoothstep(0.9, 1.0, luma) * 0.4);
  
  col = clamp(col, 0.0, 1.1);
  oC = vec4(col * vs, vs);
}`;
...
function processImage(img: HTMLImageElement) {
  ...
  // Check if the image has any transparency at all
  let hasAlpha = false;
  ...
  // Robust background detection: sample all edges
  const edgeColors: { r: number, g: number, b: number, a: number }[] = [];
  for (let x = 0; x < width; x++) {
    const top = 0 * width + x;
    const bottom = (height - 1) * width + x;
    edgeColors.push({ r: data[top * 4], g: data[top * 4 + 1], b: data[top * 4 + 2], a: data[top * 4 + 3] });
    edgeColors.push({ r: data[bottom * 4], g: data[bottom * 4 + 1], b: data[bottom * 4 + 2], a: data[bottom * 4 + 3] });
  }
  for (let y = 0; y < height; y++) {
    const left = y * width + 0;
    const right = y * width + (width - 1);
    edgeColors.push({ r: data[left * 4], g: data[left * 4 + 1], b: data[left * 4 + 2], a: data[left * 4 + 3] });
    edgeColors.push({ r: data[right * 4], g: data[right * 4 + 1], b: data[right * 4 + 2], a: data[right * 4 + 3] });
  }
  ...
  const avgA = sumA / edgeColors.length;

  const isEdgeWhite = avgR > 230 && avgG > 230 && avgB > 230;
  const isEdgeBlack = avgR < 25 && avgG < 25 && avgB < 25;
  const isEdgeTransparent = avgA < 50;
  ...
  for (let i = 0; i < size; i++) {
    ...
    if (isEdgeTransparent) {
      isBackground = a < 128; // If edge is transparent, trust alpha
    } else if (isEdgeWhite) {
      // Key out white if edge is white
      isBackground = r > 235 && g > 235 && b > 235;
    } else if (isEdgeBlack) {
      // Key out black if edge is black
      isBackground = r < 30 && g < 30 && b < 30;
    } else {
      // Fallback: key out very transparent pixels
      isBackground = a < 50;
    }
...
export default function MetallicPaint({
  ...
  return (
    <canvas 
      ...
      style={{ opacity: textureReady ? 1 : 0, transition: 'opacity 0.5s ease' }}
    />
  );
}
